...
function renderMetrics(filteredCount=masterSchedule.length){
  ...
  const metrics=[
    ...
    ['Provisional scope',nfmt(provisional),'WHERE_APPLICABLE · verify IFC / BOQ / equipment schedule',provisional?'scopewarn':'ok'],
...
function filteredRows(){
  ...
  return masterSchedule.filter(r=>{
    ...
    if(q){const hay=[r.activity_id,r.wbs,r.activity_name,r.building_area,r.discipline,r.responsible_party,r.deliverable_evidence,r.source_reference,r.basis_type,r.timing_basis,r.scope_applicability,r.scope_note,r.computed_total_float_days,r.network_from_start,r.network_to_final].join(' ').toLowerCase();if(!hay.includes(q))return false;}
    return true;
  });
}
...
function leftTask(r){
  ...
  div.innerHTML=`<div class="cell">${esc(r.wbs)}</div><div class="cell name"><div><div class="title">${esc(r.activity_name)} ${basisChip(r)} ${timingChip(r)} ${scopeChip(r)} ${networkChip(r)}</div><div class="sub">${esc(r.activity_id)} · ${esc(r.building_area)} · ${esc(r.discipline)}${esc(fl)}</div></div></div><div class="cell">${r.milestone==='Y'?'MS':esc(r.duration_days+'d')}</div><div class="cell">D${r.start_day}–${r.finish_day}</div>`;return div;
}
function timeTask(r){
