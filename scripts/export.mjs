...
const provisional=masterSchedule.filter(r=>r.scope_applicability==='WHERE_APPLICABLE');
const provisionalFields=[
  ...
  'scope_applicability','scope_note','source_reference','notes'
];
...
const documentGates=masterSchedule.filter(r=>r.plan_no==='11' && r.discipline==='Controlled Document / Engineering Gate');
const documentFields=[
  'activity_id','wbs','zone','building_area','activity_name','start_day','predecessor','relationship','lag_days',
  'deliverable_evidence','timing_basis','source_reference','notes'
];
fs.writeFileSync('data/package-document-release-register.csv',toCsv(documentGates,documentFields));

const testPacks=masterSchedule.filter(r=>r.plan_no==='01' && /-(?:ETST|PTST|HTST|ITST|KTST|WTST|CTST)$/.test(r.activity_id));
const testFields=[
  'activity_id','wbs','zone','building_area','discipline','activity_name','start_day','finish_day','predecessor','relationship',
  'deliverable_evidence','scope_applicability','scope_note','source_reference','notes'
];
...
console.log(`Provisional scope register: ${provisional.length} rows.`);
console.log(`Package document release register: ${documentGates.length} rows.`);
console.log(`System test pack register: ${testPacks.length} rows.`);
