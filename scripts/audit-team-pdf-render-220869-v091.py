...
ROOT = Path('data')
PAGES_DIR = ROOT / 'team-pdf-pages-220869-v091'
REPORT_JSON = ROOT / 'team-pdf-render-audit-220869-v091.json'
REPORT_MD = ROOT / 'team-pdf-render-audit-220869-v091.md'

EXPECTED_COLORS = {
    'work_gray': (217, 217, 217),
    'zone_blue': (219, 234, 254),
    'activity_green': (236, 253, 243),
    'tbc_yellow': (255, 246, 216),
    'control_window_green': (238, 248, 239),
    'critical_exposure_red': (196, 61, 61),
}
...
for index, path in enumerate(page_paths, start=1):
    ...
    page_metric = {
        ...
        'semantic_color_pixels': page_colors,
    }
    if ratio < 0.012:
...
minimum_color_pixels = {
    'work_gray': 10000,
    'zone_blue': 8000,
    'activity_green': 15000,
    'tbc_yellow': 2500,
    'control_window_green': 2500,
    'critical_exposure_red': 100,
}
for name, minimum in minimum_color_pixels.items():
    if color_totals[name] < minimum:
        errors.append({
            'issue': 'semantic-color-missing-or-too-low',
            'color': name,
            'minimum': minimum,
            'actual': color_totals[name]
        })

# Build contact sheets for visual review. These are QA artifacts, not PDF content.
...
for sheet_index in range(math.ceil(len(page_paths) / (cols * rows))):
    ...
    output = ROOT / f'team-pdf-contact-sheet-220869-v091-{sheet_index + 1:02d}.png'
...
report = {
    ...
    'semantic_color_pixels': color_totals,
    'errors': errors,
...
md = [
    '# Team Gantt PDF Raster Audit — Excel 220869 v0.9.1', '',
    ...
    f'- Semantic colors: `{json.dumps(color_totals, ensure_ascii=False)}`',
...
print(f'Team PDF v0.9.1 raster audit: {report["status"]}')
...
print(f'Semantic color pixels: {json.dumps(color_totals)}')
