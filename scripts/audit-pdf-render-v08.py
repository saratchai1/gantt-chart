...
ROOT = Path('data')
PAGES_DIR = ROOT / 'pdf-v08-pages'
REPORT_JSON = ROOT / 'pdf-render-audit-v08.json'
REPORT_MD = ROOT / 'pdf-render-audit-v08.md'
...
for index, path in enumerate(page_paths, start=1):
    ...
    # Page 1 intentionally has a full-width top banner. Detailed pages must
    # retain clear right and bottom margins rather than clipping content.
    if index >= 2:
        ...
        # Geometry at 72 dpi approximately matches PDF points. Scale the
        # expected table/timeline zones when Poppler returns a different DPI.
        sx = width / 1191.0
        sy = height / 842.0
        body_top = int(58 * sy)
        body_bottom = int(815 * sy)
        table_right = int(670 * sx)
...
# Build 4x2 contact sheets for human review without changing the PDF itself.
...
for sheet_index in range(math.ceil(len(page_paths) / (cols * rows))):
    ...
    output = ROOT / f'pdf-v08-contact-sheet-{sheet_index + 1:02d}.png'
...
md_lines = [
    '# PDF Raster Audit — Baseline v0.8 ภาษาไทย',
    '',
...
print(f'PDF raster audit status: {report["status"]}')
